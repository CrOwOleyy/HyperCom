                        ...
                        std::size_t index)
{
    ...
    // The status dot lives below the button rather than inside it: a
    // button colored by state would be unreadable for anyone who can't
...
void draw_server_bar(app_state &app, server_slot_list &slots, float width)
{
    ...
    if (!app.status_message.empty()) {
        ImGui::PushStyleColor(
            ImGuiCol_Text, app.status_is_error ? AERO_ALERT : AERO_INK_MUTED);
