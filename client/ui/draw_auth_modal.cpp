...
void draw_auth_modal(cli_context &context, ui_state &state, float scale)
{
    ...
    ImGui::Spacing();
    ImGui::TextColored(AERO_ACCENT, "%s",
                       tr("auth_welcome", state.current_lang));
    ImGui::Separator();
    ImGui::Spacing();
    ...
    bool const submit_pressed = ImGui::InputText(
        "##registration_handle", state.registration_handle_input,
    ...
    if (!state.status_message.empty()) {
        ImVec4 const color = state.status_is_error ? AERO_ALERT : AERO_ACCENT;
    ...
    if (ImGui::Button(tr("auth_btn_register", state.current_lang),
                      ImVec2{-1.0f, 40.0f * scale}) ||
        submit_pressed) {
        ...
        if (context.session.register_handle(handle_str, failure)) {
            ...
            // The only place that raises this flag: the welcome sequence
            // only plays on account creation, never on later logins.
