...
// Reproduces noise_handshake_initiator::write_first_message, with a fixed
// ephemeral key instead of a randomly drawn one.
    ...
    std::span<std::uint8_t const> payload)
{
    ...
    HYPERCOM_CHECK(
        report, crypto::compute_shared_secret(ephemeral_secret,
                                              responder_static_public, shared));
...
// Reproduces noise_handshake_responder::read_first_message.
                              ...
                              std::vector<std::uint8_t> &payload_out)
{
    ...
    HYPERCOM_CHECK(report, mix_key(state, shared));
    HYPERCOM_CHECK(report, decrypt_and_hash(
                               state, wire.subspan(remote_ephemeral_out.size()),
                               payload_out));
}

// Reproduces noise_handshake_responder::write_second_message.
[[nodiscard]] std::vector<std::uint8_t>
run_responder_second_message(test_report &report,
                             crypto::noise_symmetric_state &state,
                             crypto::x25519_secret_key const &ephemeral_secret,
...
// Reproduces noise_handshake_initiator::read_second_message.
void run_initiator_read_second(
    test_report &report, crypto::noise_symmetric_state &state,
    crypto::x25519_secret_key const &ephemeral_secret,
    std::span<std::uint8_t const> wire, std::vector<std::uint8_t> &payload_out)
{
    ...
    HYPERCOM_CHECK(report, mix_key(state, shared));
    HYPERCOM_CHECK(
        report, decrypt_and_hash(state, wire.subspan(remote_ephemeral.size()),
                                 payload_out));
}

// A single transport direction: encrypt on the sender side, decrypt on the
// receiver side, compare both against the vector.
                             ...
                             vector::wire_message const &expected)
{
    ...
    HYPERCOM_CHECK(report, receiver.decrypt_with_ad({}, expected_wire, opened));
    HYPERCOM_CHECK(report, opened == payload);
}

} // namespace

std::vector<std::uint8_t> load_bytes(test_report &report, std::string_view hex)
{
...
loaded_vector_keys load_vector_keys(test_report &report)
{
    ...
    load_fixed(report, vector::RESP_STATIC_SECRET_HEX, keys.resp_static_secret);
    ...
    HYPERCOM_CHECK(report,
                   crypto::compute_public_from_secret(keys.resp_static_secret,
                               ...
                               loaded_vector_keys const &keys)
{
    ...
    HYPERCOM_CHECK(report,
                   message1_wire ==
                       load_bytes(report, vector::MESSAGES[1].ciphertext_hex));
