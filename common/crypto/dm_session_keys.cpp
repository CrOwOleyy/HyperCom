                                          ...
                                          symmetric_key &out)
{
    ...
    bool succeeded =
        extract_pseudo_random_key(domain, material, pseudo_random);
    if (succeeded) {
                               ...
                               symmetric_key &out)
{
    ...
    bool succeeded =
        ...
        && compute_shared_secret(sender_ephemeral_secret, recipient_identity_x,
                                 second)
        && compute_shared_secret(sender_ephemeral_secret, recipient_prekey,
                                 third);
    if (succeeded) {
    ...
    x25519_public_key const &sender_ephemeral, symmetric_key &out)
{
    ...
    symmetric_key second{};
    symmetric_key third{};
    // Memes trois produits que cote expediteur, calcules dans l'autre sens.
    bool succeeded =
        compute_shared_secret(recipient_prekey_secret, sender_identity_x, first)
        && compute_shared_secret(recipient_identity_x, sender_ephemeral, second)
        && compute_shared_secret(recipient_prekey_secret, sender_ephemeral,
                                 third);
    if (succeeded) {
        succeeded = combine_shared_secrets(first, second, third, out);
    }
