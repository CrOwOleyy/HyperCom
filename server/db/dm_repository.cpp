...
bool dm_repository::insert_envelope(std::int64_t recipient_id,
                                    ...
                                    std::int64_t &out_id)
{
    sql_statement statement{
        database_,
        // No timestamp: the send date is inside the ciphertext, which the
        // server can't open. It therefore keeps no time-stamped record of
        // who's exchanging messages with whom.
                                       ...
                                       proto::dm_list_response &out)
{
    sql_statement statement{
        database_,
        // Ordered by id, not by date: the server no longer stores a
        // timestamp, and the auto-incrementing id already gives the
        // arrival order.
...
{
    // A prepared statement reused, never an IN list built by
    // concatenation. It's more verbose and it can't be injected.
    std::string error;
    ...
    for (std::uint64_t const identifier : ids) {
        ...
        // recipient_id is in the WHERE clause: nobody can delete someone
        // else's messages by guessing ids.
        succeeded =
            statement.reset_for_reuse() &&
            bind_integer(statement, 1, static_cast<std::int64_t>(identifier)) &&
...
bool dm_repository::count_pending(std::int64_t recipient_id, std::uint32_t &out)
{
    sql_statement statement{
